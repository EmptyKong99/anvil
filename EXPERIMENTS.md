...
每个实验末尾附**逐 attempt 矩阵**(行=step,列=run):格子 = 加速比(geomean vs cuBLAS,正确) · `xC`=编译/校验失败 · `xW`=结果错 · `.`=未跑;列名 b=base(no-skill)/s=skill + rep 号。由 `collect_results.py` 读 results.jsonl 生成。
