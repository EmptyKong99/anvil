...
### EXP-007 — profiler 反馈帮不帮 agent 爬?(+ 中立机制复查)  2026-06-23

**问题:** EXP-006 发现 agent 只拿 bench 速度、拿不到"为什么慢"。加了个零权限 profiler(`anvil/profile.py`:独立 `nvcc --ptxas-options=-v` 拿 regs/smem/spill,因 box 的 `RmProfilingAdminOnly=1` 用不了 ncu 计数器),把诊断回喂给模型。EXP-007 测它有没有用。

**设置:** 2 臂,**都注全 facts**(= EXP-006 instrfa 强配置),唯一变量 = profiler。`noprof` = `--no-profile`(= v1-feedback-loop 行为);`prof` = profiler 开(profiler v1 = 事实 + **处方**:"高 reg 压→降"/"occupancy 占主导")。FA、n=5、max-attempts 12,GPU 6/7 并行,同仪器。每 attempt 的 `.cu` 全归档(后面中立复查靠它)。

**结果(best per rep, % of cuDNN):**

| 臂 | rep1–5 | 中位 | 均值 | 峰值 |
|---|---|---|---|---|
| noprof | 0.20 / 1.30 / 1.61 / 14.91 / 0.49 | 1.30 | 3.70 | 14.91 |
| prof | 18.32 / 3.74 / 0.93 / 4.32 / 0.98 | 3.74 | 5.66 | 18.32 |

**⚠️ outcome 判不了(诚实):** prof 三项名义上都高,但 **n=5 + 双峰方差让中位数不可信**——两臂的高点(noprof 14.9、prof 18.3)是**同一种"开局碰巧锁进张量核盆地"的彩票**,5 抽里哪臂多中一次纯运气。硬证据:**同样"facts 无 profiler"配置,EXP-006 instrfa 中位 18.2%、这次 noprof 才 1.3%(差 14×)**——批次间方差 ≫ 组间差。所以不能凭中位数说 profiler 有用。

**中立机制复查(避开 outcome 彩票,问"行为变没变 + okbench 奖不奖励",不问"听没听我的"):** 重编全 10 rep 的每个 attempt 拿资源轨迹:

| | mean\|Δregs\|/step | mean\|Δsmem\| | regs动≥4的步→Δbench | regs几乎不动的步→Δbench |
|---|---|---|---|---|
| noprof | 21.7 | 6.5 KB | +0.61% (n=4) | +0.50% (n=10) |
| prof | 24.5 | **13.2 KB** | **−0.29% (n=8)** | **+2.34% (n=7)** |

1. **profiler 改变了行为**(没被无视):prof 挪 smem 2×、regs 略多。
2. **但它引导的方向不挣钱:** prof 里**大动寄存器的步反而略降(−0.29%),涨分来自"没碰寄存器、改别的"的步(+2.34%)**。即 profiler 的**处方("降 reg / occupancy 占主导")对 FA 是误导**——FA 靠 cp.async 流水在低占用也能快(forge v7 56% 即是),不是靠堆 occupancy。

**结论:** **没有证据表明(带处方的)profiler 帮了;反而有(弱、n 小)证据它把努力往错方向带。** 这验证了把 profiler 改成 **facts-first**(只喂 regs/smem/spill + 中立换算,删处方,标"not advice";anvil dev `6ee986d`)的决定。更重要:**profiler 是盆地内杠杆,管不了"标量→张量核"那一跃,所以即便完美也难抬中位数**;抬中位数的杠杆是给对起跑盆地 / basin-breaker。

**强度:** 中立指标 n=7/8,提示性非定论。下一步:① facts-first profiler 复测;② 真正打中位数的——**seed 一个相同的正确张量核起点**(把"发明结构"换成"优化结构",直击 EXP-006 的盆地锁定)。

**产物:** `runs_exp007/{noprof,prof}/...`;基建 = `profile.py` + `OKBenchRunner.profile_enabled` + cli `--no-profile`,每 attempt 的 `.cu` 归档使事后中立复查成为可能。
