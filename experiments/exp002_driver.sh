...
run_arm() {
  ...
  for r in 1 2 3; do
    echo "[driver] $arm rep$r start $(date)" >> runs_exp002/driver.log
    .venv/bin/python -m anvil.cli agent --op gemm_bf16_nt \
      --repo /nvme/share/gucheng/OpenKernels --device "$dev" \
      --author "gucheng_${arm}" --max-attempts 12 --out-dir "runs_exp002/$arm" "$@" \
      > "runs_exp002/${arm}_rep${r}.log" 2>&1
    echo "[driver] $arm rep$r done  $(date)" >> runs_exp002/driver.log
  done
}
run_arm base  6 &
run_arm skill 7 --inject-skill &
wait
echo "done $(date)" > runs_exp002/DONE
